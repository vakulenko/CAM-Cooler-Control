...
namespace cooler_control_01
{
    public partial class main_form : Form
    {
        ...
        private double air_temp=0.0, sensor_temp=0.0, val_temp=0.0;
        ...
        private byte read_packet()
        {
            ...
            switch (rx_buf[0])
            {
                ...
                case 0x64:  {
                            ...
                            if (pwm_state == 0x00)
                            {
                                onoff_button.Text = "Cooler on";
                                onoff_button.Enabled = true;
                            }
                            else
                            {
                                onoff_button.Text = "Cooler off";
                                onoff_button.Enabled = true;
                            }

                            switch (err_code)
                                {
                                case 0: {   error_label.Text = "No Errors";
                                            power_label.Text = (pwm * 100 / 255).ToString() + "%";
                                            externaltemp_label.Text = air_temp.ToString("F1") + "C";
                                            internaltemp_label.Text = sensor_temp.ToString("F1") + "C";
                                            deltatemp_label.Text = (air_temp - sensor_temp).ToString("F1") + "C";
                                            settemp_label.Text = val_temp.ToString("F1") + "C";
                                            if (pwm_state==0) status_label.Text = "Off";
                                            else status_label.Text = "On";
                                            break;
                                        }
                                case 1: {   error_label.Text = "Internal sensor failed";
                                            power_label.Text = (pwm * 100 / 255).ToString() + "%";
                                            externaltemp_label.Text = air_temp.ToString("F1") + "C";
